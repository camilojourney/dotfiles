...
while IFS=$'\t' read -r name target; do
  ...
  actual_name="$(/usr/libexec/PlistBuddy -c 'Print :CFBundleName' "$app/Contents/Info.plist")"
  [[ "$actual_name" == "Cursor $name" ]] || { echo "Wrong bundle name: $app" >&2; exit 1; }
  osadecompile "$app/Contents/Resources/Scripts/main.scpt" | grep -Fq "quoted form of \"$target\"" || {
